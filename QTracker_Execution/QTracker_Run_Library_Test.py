...
def process_file(file_path, file_extension, max_ele, dimuon_prob_threshold):
    try:
        if file_extension == '.root':
            # Read and process data from ROOT file
            targettree = uproot.open(file_path + ":QA_ana;1")
            ...
            print("Hits instanteated")
            drift = np.zeros((len(detectorid), 54, 201))

            for n in range(len(detectorid)):
                hits[n], drift[n], _ = hit_matrix(
                    detectorid[n], elementid[n], driftdistance[n], tdctime[n], intime[n], hits[n], drift[n], None
                )

        elif file_extension == '.npz':
            data = np.load(file_path)
            hits, drift = data["hits"], data["drift"]

        print("Loaded events")

        # Filter events based on dimuon probability
        # model = load_model('Networks/event_filter')
        # prob_model = tf.keras.Sequential([model, tf.keras.layers.Softmax()])
        # probabilities = prob_model.predict(hits, batch_size=256, verbose=0)

        # valid_events = probabilities[:, 1] > dimuon_prob_threshold
        # hits, drift = hits[valid_events], drift[valid_events]

        print("Filtered Events")

        if hits.size > 0:
            # Perform track finding and reconstruction
            track_model = load_model('Track_Finder_All')
            predictions = (np.round(track_model.predict(hits, verbose=0) * max_ele)).astype(int)
            tracks = evaluate_finder(hits, drift, predictions)[:, :, :2]

            recon_model = load_model('Reconstruction_All')
            reco_kinematics = recon_model.predict(tracks, batch_size=8192, verbose=0)

            print("Reconstruction complete for", len(reco_kinematics), "events.")
        else:
