...
{
  const double target_coil_pos_z = -125;
  const int nmu = 1;
  int embedding_opt = 0;
  ...
  const double target_l = 50.8; //cm
  const double target_z = -target_l/2.; //cm
  ...
  if(gen_pythia8) {    
    ...
    pythia8->set_vertex_distribution_mean(0, 0, 65, 0);
    ...
    pythia8->set_vertex_distribution_width(1.,1.,40,0);
    ...
    trigger_mup->SetPzHighLow(120, 5);
  ...
  SetupBeamline(g4Reco, do_collimator, - 602.36); // Is the position correct??
