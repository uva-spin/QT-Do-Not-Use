...
X_train, X_test, y_train, y_test = train_test_split(hitmatrix, Truth_values_elemID, test_size=0.25, random_state=42, shuffle=True)
...
hist = model.fit(X_train, y_train, epochs=50, batch_size=32,validation_data=(X_test,y_test),callbacks=[callback])
...
# hit_arrays = np.reshape(hit_arrays,(number_of_events,49,2))
# test = np.reshape(y_test,(number_of_events,49,2))
# print(hit_arrays[0][:,0].astype(int))

# Model_Track_1_elemID = hit_arrays[0][:,0]
# Test_Track_1_elemID = test[0][:,0]
# Model_Track_2_elemID = hit_arrays[:,0]
# Test_Track_2_elemID = test[:,0]
# plt.clf()
# plt.plot(Model_Track_1_elemID,color = 'r')
# plt.plot(Test_Track_1_elemID)
